...
// Wraps and AudioMixerGroup in Unity's AudioMixer. Constains the name of the group 
// (which is alos its exposed valume paramter), the group itself and an IEnumerator
// for doing track fades over time.
...
// Provides pooled one-shot functionality with priority system and also wraps the
// Unity Audio Mixer to make easier manipulation of audiogroup volumes
public class AudioManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        // Fetch all the groups in the mixer - These will be our mixers trakcs
    ...
    // Return the volume of the AudioMixerGroup assign to the passed track.
    // AudioMixerGroup must expose its volume variable to script for this to work 
    // and the variable must be the same as the name of the group
    ...
    public void StopOneShotSound(ulong id)
    {
        ...
        if (_activePool.TryGetValue(id, out activeSound))
        {
            ...
            // Stop the sound manually and disabe the game object
            activeSound.AudioSource.Stop();
            activeSound.AudioSource.clip = null;
            activeSound.GameObject.SetActive(false);
    ...
    // Scores the priority of the sound and searches for an unused pool item
    // to use as the audio source. If one is not available and an audio source
    ...
    public ulong PlayOneShotSound( string track, AudioClip clip, Vector3 position, float volume, float spatialBlend, int priority = 128)
    {
        ...
        for (int i = 0; i < _pool.Count; i++)
        {
            ...
            if (poolItem.Unimportance > leastImportantIndex)
            {
    ...
    public IEnumerator PlayOneShotSoundDelayed(string track, AudioClip clip, Vector3 position, float volume, float spatialBlend,
        float duration, int priority = 128)
    {
        yield return new WaitForSeconds ( duration );
