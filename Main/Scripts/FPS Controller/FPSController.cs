...
public class CurveControlledBob
{
    ...
    {
        // Record time lenght of bob curve
            ...
            if(ev!=null)
            {
                if (ev.Type == CurveControlledBobCallbackType.Vertical)
                {
                    if ((_prevYPlayHead < ev.Time && _yPlayHead >= ev.Time)       ||
                        (_prevYPlayHead > _yPlayHead && (ev.Time > _prevYPlayHead || 
                         ev.Time <= _yPlayHead)))
                    {
                        ev.Function();
                    }                
...
public class FPSController : MonoBehaviour
{
    ...
    [SerializeField] private float              _runStepLengthen    = 0.65f;
    ...
    // Public Properties
    public PlayerMoveStatus movemenStatus { get { return _movementStatus; } }
    ...
    public float dragMultiplier
    {
        ...
        set { _dragMultiplier = Mathf.Min( value, _dragMultiplier ); }
    }
    ...
    protected void Start()
    {
        ...
        // Get the main camera anc cache local position within the fps rig
        _camera = Camera.main;
        ...
        _headBob.Initialize();
        _headBob.RegisterEventCallback(1.1f, PlayFootStepSound, CurveControlledBobCallbackType.Vertical);  // Also do not 
    ...
    protected void FixedUpdate()
    {
        ...
        // Always move along the camera forwards as it is the direction that it being aimed at
        ...
        if (_characterController.isGrounded)
        {
            ...
            // If the jump button was pressed the apply speed in up direction
        ...
        {
            // Otherwise we are not on the ground so apply standart system gravity multiplied
    ...
    //// We got different methods to use it and it will disable asap
    //public List<AudioSource> AudioSources = new List<AudioSource>();
    //private int _audioToUse = 0;
    //void PlayFootStepSound() { if(_isCrouching) return; AudioSources[_audioToUse].Play(); _audioToUse = (_audioToUse == 0) ? 1 : 0; }
    //

    // This method used for enemies collider will not push us backwards via collider
