...
public class AudioCollectionPlayer : AIStateMachineLink
{
    ...
    // Called by the animation system for each frame update of the animation 
    public override void OnStateUpdate(Animator animator, AnimatorStateInfo animStateInfo, int layerIndex)
    {
        ...
        int customCommand = ( _customCurve == null ) ? 0 : Mathf.FloorToInt ( _customCurve.Evaluate( 
            animStateInfo.normalizedTime - (long)animStateInfo.normalizedTime ));
