    ...
    public override AIStateType OnUpdate()
    {
        ...
        if ( currentHash == _eatingStateHash || currentHash == _crawlEatingStateHash)
        {
            _zombieStateMachine.satisfaction = Mathf.Min(_zombieStateMachine.satisfaction 
                                                      + ((Time.deltaTime * _zombieStateMachine.replenishRate)/100.0f),1.0f);
        ...
        if ( !_zombieStateMachine.useRootRotation)
        {
            ...
            _zombieStateMachine.transform.rotation = Quaternion.Slerp ( _zombieStateMachine.transform.rotation, 
                                                                        newRot, Time.deltaTime * _slerpSpeed);
        }


        // Calculated zombie's mouth to dead body foods within the different situations
        ...
        if ( currentHash == _eatingStateHash )
        {
        ...
        if ( currentHash == _crawlEatingStateHash )
        {
            _zombieStateMachine.transform.position = Vector3.Lerp(_zombieStateMachine.transform.position,
                                                                  _zombieStateMachine.transform.position + headToTarget - new Vector3(0, -0.1f, 0.0f),
                                                                  1.2f * Time.deltaTime);
        }
