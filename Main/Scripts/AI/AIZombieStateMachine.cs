...
// --------------------------------------------------------------------------
// BodyPartSnapshot
// Used to store information about the position of
// each body part when transitioning from a ragdoll
// --------------------------------------------------------------------------
...
public class AIZombieStateMachine : AIStateMachine
{
    ...
    protected IEnumerator Reanimate()
    {
        ...
        // Set rigidboies back to being kinematic
        foreach (Rigidbody body in _bodyParts)
        {
        ...
        // Record positions and rotation of all bones prior to reanimation
        ...
        if (_rootBone != null)
        {
            ...
            {
                case AIBoneAligmentType.ZAxis:
                    forwardTest = _rootBone.forward.y;  break;
                case AIBoneAligmentType.ZAxisInverted:
                    forwardTest = -_rootBone.forward.y; break;
                case AIBoneAligmentType.YAxis:
                    forwardTest = _rootBone.up.y;       break;
                case AIBoneAligmentType.YAxisInverted:
                    forwardTest = -_rootBone.up.y;      break;
                case AIBoneAligmentType.XAxis:
                    forwardTest = _rootBone.right.y;    break;
                case AIBoneAligmentType.XAxisInverted:
                    forwardTest = -_rootBone.right.y;   break;
    ...
    // LateUpdate
    // Called by Unity at the end of every frame update.
    // Used here to perform reanimation
    protected virtual void LateUpdate()
    {
        if (_boneControlType == AIBoneControlType.RagdollToAnim)
        {
            if (Time.time <= _ragdollEndTime + _mecanimTransitionTime)
            {
                ...
                // Assigned baseOfset value and fethced from navAgent system. 
                ...
                // Try to match rotations. Note that we can only rotate around Y axis, as the animated characted must stay upright,
